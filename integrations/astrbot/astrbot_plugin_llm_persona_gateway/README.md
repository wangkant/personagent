...
3. Keep NapCat's HTTP API reachable at the agent's `NAPCAT_API`. The
   catch-up sweep for missed mentions goes through it directly, and so do
   proactive messages on an agent without the [outbox](#outbox). An agent
   with it sends them through this plugin.
...
| WeCom smart bot (`wecom_ai_bot`) | Groups (which WeCom delivers only when the bot is @-ed) and DMs. Voice, file and video notes. A turn sent as one streamed answer. | Replies only inside the answer to a message: no outbox, no mentions. Set `wecom_ai_bot_name` in AstrBot so "@bot" is removed from the text. |
